...
export default function RecommendationCard({ device, onClose }: RecommendationCardProps) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="bg-white rounded-[2rem] shadow-2xl overflow-hidden text-[#1a1c23] p-6 md:p-8 flex flex-col relative max-w-2xl">
          <button 
            onClick={onClose}
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-500 mb-6 font-medium">Consumo estimado: <span className="font-mono font-bold text-gray-800">{device.consumoEstimado}</span></p>
            
            <div className="space-y-4 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
              {device.recomendaciones.map((tip, idx) => (
                <div key={idx} className="flex items-start gap-4 p-3 bg-gray-50 rounded-xl border border-gray-100">
